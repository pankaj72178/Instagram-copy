...
export default async function FollowingPage({
  ...
  if (canView) {
    ...
    const states = await followStatesFor(viewerId, following.map((f) => f.id));
  ...
  return (
    <main className="mx-auto w-full max-w-md flex-1 px-4 py-6">
      <header className="mb-4">
        ...
        <h1 className="mt-1 text-xl font-bold">Following</h1>
      </header>
      {canView ? (
        <UserList users={users} emptyText="Not following anyone yet." />
      ) : (
